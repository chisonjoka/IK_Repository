 <?php
	...
	if($len > 0){
		...
		$query ="select contaid from contacts order by contaid desc limit 1";
		$ref = mysql_query($query, $connection) or die (mysql_error());

		$query = "INSERT INTO categories(catid, name) VALUES(NULL,'$category')";
		mysql_query($query, $connection) or die (mysql_error());
		
		$query = "INSERT INTO knowledge(kid, kname, kdetails, kimageurl, catid, contrid) VALUES(NULL,'$name','$description','$image_url', (select catid from categories order by catid desc limit 1),(select contrid from contributors order by contrid desc limit 1))";
